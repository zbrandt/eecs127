...
\textbf{Example convex functions}: $f(x) = e^{ax}$,
$f(x) = x^a$ where $a \geq 1$ or $a \leq 0$ on $\R_{++}$,
$f(x) = -\log(x)$ on $\R_{++}$, any $\ell_p$ form function
$f(x) = \|x\|_p$, quadratic functions
$f(x) = x^\top P x + q^\top x + r$ where $P$ is symmetric
and $P \succeq 0$. If $P \succ 0$, $f$ is strictly convex.
